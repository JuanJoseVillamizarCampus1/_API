...
const { postDenuncia, getDenunciasAnonimas, deletedenuncia,getDenunciasTotales } = require('../controllers/denuncia.controllers');
...
routerDenunica.get('/',[permisosRol,validateDocuments],getDenunciasAnonimas)//GET activas//http://localhost:8001/api/denuncia-anonima?page=1&perPage=10
...
//DELETE
routerDenunica.delete('/:id',[validateJWT,permisosRol,check('id', 'No es un ID válido').isMongoId(),
check('id').custom(userExistsById),validateDocuments],deletedenuncia) //DELETE//http://localhost:8001/api/login/ID
